...
function getInstallProperty() {
	...
	for file in "${COMPONENT_INSTALL_ARGS}" "${INSTALL_ARGS}"; do
		if [ -f "${file}" ]; then
...
echo "xxx HCOMPONENT_INSTALL_DIR_NAME: ${HCOMPONENT_INSTALL_DIR_NAME}"
...
if [ "${HCOMPONENT_INSTALL_DIR_NAME}" = "" ]; then
...
echo "xxx HCOMPONENT_LIB_DIR: ${HCOMPONENT_LIB_DIR}"
if [ "${HCOMPONENT_NAME}" = "knox" ]; then
...
echo "xxx HCOMPONENT_INSTALL_DIR: ${HCOMPONENT_INSTALL_DIR}"
...
if [ ! -d "${HCOMPONENT_INSTALL_DIR}" ]; then
...
if [ "${HCOMPONENT_NAME}" = "hive" ]; then
	HCOMPONENT_CONF_SERVER_DIR="${HCOMPONENT_CONF_DIR}"/conf.server
	if [ -d "${HCOMPONENT_CONF_SERVER_DIR}" ]; then
...
if [ -f "${SET_ENV_SCRIPT_TEMPLATE}" ]; then
	#
...
if [ -d "${PROJ_INSTALL_DIR}/install/conf.templates/${action}" ]; then
	...
	if [ "${action}" = "enable" ]; then
		...
		for cf in ${PROJ_INSTALL_DIR}/install/conf.templates/${action}/*.xml; do
			cfb=$(basename ${cf})
			if [ -f "${HCOMPONENT_CONF_DIR}/${cfb}" ]; then
		...
		if [ -f ${HCOMPONENT_CONF_DIR}/ranger-security.xml ]; then
	...
	for f in ${PROJ_INSTALL_DIR}/install/conf.templates/${action}/*.cfg; do
		if [ -f "${f}" ]; then
			fn=$(basename $f)
			orgfn=$(echo $fn | sed -e 's:-changes.cfg:.xml:')
			fullpathorgfn="${HCOMPONENT_CONF_DIR}/${orgfn}"
			if [ ! -f ${fullpathorgfn} ]; then
				if [ -f ${DEFAULT_XML_CONFIG} ]; then
	...
	if [ "${HCOMPONENT_NAME}" = "hbase" ] || [ "${HCOMPONENT_NAME}" = "storm" ]; then
...
if [ "${action}" = "enable" ]; then
	dt=$(date '+%Y%m%d%H%M%S')
	for f in ${PROJ_LIB_DIR}/*; do
		if [ -f "${f}" ] || [ -d "${f}" ]; then
			bn=$(basename $f)
			if [ -f ${HCOMPONENT_LIB_DIR}/${bn} ] || [ -d ${HCOMPONENT_LIB_DIR}/${bn} ]; then
...
if [ "${HCOMPONENT_NAME}" = "knox" ]; then
	...
	dt=$(date '+%Y%m%d%H%M%S')
	for fn in $(ls ${HCOMPONENT_CONF_DIR}/topologies/*.xml 2>/dev/null); do
		if [ -f "${fn}" ]; then
			dn=$(dirname ${fn})
			bn=$(basename ${fn})
			bf=${dn}/.${bn}.${dt}
...
addPropertyToFile() {
	...
	if test -z "$validate"; then
		log "[E] Failed to add properties '$1' to $3 file!"
		exit 1
	fi
...
updatePropertyToFile() {
	...
	validate=$(sed '/^\#/d' $3 | grep "^$1" | tail -n 1 | cut -d "=" -f2-) # for validation
	if test -z "$validate"; then
		log "[E] '$1' not found in $3 file while Updating....!!"
		exit 1
	fi
...
if [ "${HCOMPONENT_NAME}" = "presto" ]; then
	if [ "${action}" = "enable" ]; then
		controlName="ranger"
	else
